...
This guide demonstrates how to generate VTK visualization files from Kynema-FMB simulation
...
.. tip::
   ...
   .. code-block:: text
      ...
      Generate VTK files from Kynema-FMB NetCDF output

      positional arguments:
        netcdf_file           Path to Kynema-FMB NetCDF output file e.g. blade_interface.nc
...
The ``generate_vtk_output.py`` script is a post-processing tool that converts Kynema-FMB simulation
...
2. **Element Visualization**: Creates appropriate VTK representations for different Kynema-FMB elements:
...
- ``netCDF4``: For reading Kynema-FMB NetCDF output files
...
simulations via Kynema-FMB APIs found in ``src/interfaces``.

1. **NetCDF Output File**: The primary output file generated by Kynema-FMB during simulation.
   ...
   Kynema-FMB APIs provided in ``src/interfaces``. File naming follows this pattern:
...
- ``netcdf_file``: Path to Kynema-FMB NetCDF output file (e.g., ``turbine_interface.nc``)
- ``connectivity_file``: Path to mesh connectivity YAML file (e.g., ``mesh_connectivity.yaml``)
...
generated by the VTK script by leveraging the Kynema-FMB interfaces/APIs via the regression
tests.
...
.. note::
   ...
   created to demonstrate the robustness of the Kynema-FMB model, even under fast, large nonlinear
   deflections.
