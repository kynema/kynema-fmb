...
TEST(AerodynamicsComponentTest, CalculateAngleOfAttack_Case1) {
    ...
    const auto aoa = kynema_fmb::interfaces::components::CalculateAngleOfAttack(v_rel);
...
TEST(AerodynamicsComponentTest, CalculateAerodynamicLoad_Case1) {
    ...
    auto load = kynema_fmb::interfaces::components::CalculateAerodynamicLoad(
...
TEST(AerodynamicsComponentTest, CalculateConMotionVector_Case1) {
    ...
    const auto ac_vector = kynema_fmb::interfaces::components::CalculateConMotionVector(
...
TEST(AerodynamicsComponentTest, CalculateJoacibanXi_Case1) {
    ...
    const auto jacobian_xi = kynema_fmb::interfaces::components::CalculateJacobianXi(aero_node_xi);
...
TEST(AerodynamicsComponentTest, CalculateAeroNodeWidths_Straight) {
    ...
    for (auto i = 0U; i < jacobian_xi.size(); ++i) {
        kynema_fmb::math::LagrangePolynomialDerivWeights(jacobian_xi[i], beam_node_xi, weights);
        for (auto j = 0U; j < beam_node_xi.size(); ++j) {
            jacobian_integration_matrix[(i * beam_node_xi.size()) + j] = weights[j];
        }
    }

    const auto widths = kynema_fmb::interfaces::components::CalculateAeroNodeWidths(
